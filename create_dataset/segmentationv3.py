...
if __name__ == '__main__':
  ...
  if not os.path.exists("{}_pp".format(args.dataset_name)):
    ...
    texts = os.listdir(args.dataset_name)
  else:
    texts = os.listdir(args.dataset_name)
    aux =  os.listdir("{}_pp".format(args.dataset_name))
    aux = [i.replace('.json', '.xml') for i in aux]
    texts = texts[texts.index(aux[-1]):]

  Parallel(n_jobs=10)(delayed(main)(file_name, args.dataset_name) for file_name in texts)
